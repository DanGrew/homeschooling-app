(function () {
  ...
  ann.segments.forEach(function (seg) {
    seg.words.forEach(function (w) { allWords.push(w); });
  });

  var area = document.getElementById('player-area');
  var wordsHtml = '';
  allWords.forEach(function (w, i) {
    wordsHtml += '<span id="w' + i + '" style="display:inline-block;padding:4px 8px;border-radius:10px;transition:background 0.08s,color 0.08s;">' + w.w + '\u00a0</span>';
  });
  ...
  area.innerHTML =
    ...
    '<div id="words" style="font-size:2em;line-height:2;text-align:center;margin-bottom:28px;min-height:100px;">' + wordsHtml + '</div>' +
