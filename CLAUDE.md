...
**Run the app locally (no IntelliJ needed) — serve from the worktree you're working in, so what you see is *that worktree's* code. `cd` into the worktree first, then run `test-server.js`:**
```bash
cd /abs/path/to/<worktree> && PORT=3001 node test-server.js     # → http://localhost:3001/
```
Static site, no build step. `test-server.js` serves **its own directory** (`public: __dirname`) and `chdir`s to it on startup via `serve-handler`, so the directory served is wherever the *script file* lives. It strips the `/homeschooling-app` GitHub-Pages path prefix, so both `http://localhost:3001/` and `http://localhost:3001/homeschooling-app/...` resolve (the latter matches the deployed Pages URL). Open `/` → redirects to `app/`. First run needs `npm install` (pulls `serve-handler` via the `serve` dep).

(A `cd` in a compound command can trigger a one-off permission prompt the first time; approve it. If you'd rather skip the `cd` entirely, the equivalent no-`cd` form is `PORT=3001 node /abs/path/to/<worktree>/test-server.js` — because the script `chdir`s to its own dir, pointing `node` at the worktree's `test-server.js` serves that worktree regardless of your shell's cwd.)
...
```bash
# start — cd into the worktree, then run its test-server.js:
