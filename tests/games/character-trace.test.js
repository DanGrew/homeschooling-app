...
test('page loads with ball and first character label', async ({ page }) => {
  await page.goto('/app/games/character-trace.html')
...
test('next button advances character', async ({ page }) => {
  await page.goto('/app/games/character-trace.html?char=a&filter=lower')
...
test('prev button goes back', async ({ page }) => {
  await page.goto('/app/games/character-trace.html?char=b&filter=lower')
...
test('URL params set initial character and filter', async ({ page }) => {
  await page.goto('/app/games/character-trace.html?char=m&filter=lower')
  const label = await page.locator('#char-label').textContent()
  expect(label).toBe('m')
