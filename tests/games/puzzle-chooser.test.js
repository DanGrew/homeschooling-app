...
test('chooser loads showing puzzle card with thumbnail', async ({ page }) => {
  await page.goto(CHOOSER_URL)
  await page.waitForSelector('.puzzle-card')
  await expect(page.locator('.puzzle-card')).toHaveCount(1)
  await expect(page.locator('.puzzle-card img')).toHaveAttribute('src', /paw-patrol/)
})

test('single puzzle auto-selects — grid picker visible on load', async ({ page }) => {
  await page.goto(CHOOSER_URL)
  await page.waitForSelector('#grid-picker.visible')
  await expect(page.locator('#grid-picker')).toBeVisible()
})
