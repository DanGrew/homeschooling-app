...
test('filter bar rendered with All option active', async ({ page }) => {
  await page.goto(URL)
  await expect(page.locator('#nav-filter-slot')).toBeVisible({ timeout: 3000 })
...
test('filter bar has a-z, A-Z, 0-9 options', async ({ page }) => {
  await page.goto(URL)
  await expect(page.locator('#nav-filter-slot button[data-value="lower"]')).toBeVisible({ timeout: 3000 })
  await expect(page.locator('#nav-filter-slot button[data-value="upper"]')).toBeVisible()
  await expect(page.locator('#nav-filter-slot button[data-value="digit"]')).toBeVisible()
})

test('clicking a-z filter activates it', async ({ page }) => {
  await page.goto(URL)
  await page.locator('#nav-filter-slot button[data-value="lower"]').click({ timeout: 3000 })
  const btn = page.locator('#nav-filter-slot button[data-value="lower"]')
