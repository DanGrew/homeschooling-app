(function () {
  ...
  function renderCard() {
    var p = prompts[idx];
    var col = TYPE_COLOR[p.type] || '#999';
    var lbl = TYPE_LABEL[p.type] || p.type.toUpperCase();
    card.innerHTML =
      ...
      '<div style="padding:14px 14px 10px;">' +
        ...
        '<div style="font-size:0.92em;color:#333;line-height:1.55;">' + p.text + '</div>' +
      '</div>' +
      '<div style="padding:8px 12px 12px;display:flex;align-items:center;justify-content:space-between;border-top:1px solid #f0f0f0;">' +
        '<div id="ap-prev" style="width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#ccc;font-size:1.3em;border-radius:8px;background:#f5f5f5;line-height:1;">&#8249;</div>' +
        '<span style="font-size:0.75em;color:#ccc;">' + (idx + 1) + '\u202f/\u202f' + prompts.length + '</span>' +
        '<div id="ap-next" style="width:34px;height:34px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#ccc;font-size:1.3em;border-radius:8px;background:#f5f5f5;line-height:1;">&#8250;</div>' +
      '</div>';

    card.querySelector('#ap-close').addEventListener('click', closeCard);
    card.querySelector('#ap-prev').addEventListener('click', function () { idx = (idx - 1 + prompts.length) % prompts.length; renderCard(); });
    card.querySelector('#ap-next').addEventListener('click', function () { idx = (idx + 1) % prompts.length; renderCard(); });
  }
